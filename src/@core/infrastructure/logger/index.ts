...
const formatCustom = format.printf((data: any) => {
	...
	return `${level} ${timestamp}: ${typeof message === 'string' ? message : util.inspect(message, { colors: true })}`;
});
...
if (process.env.WRITE_LOG_FILE === 'true') {
	...
	const consoleTransport = new Console({
		format: format.combine(format.colorize(), format.simple(), format.timestamp({ format: 'DD/MM/YYYY HH:mm:ss' }), errorStackFormat(), formatCustom),
	});
